/**
 * SellAuth
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.SellAuth);
  }
}(this, function(expect, SellAuth) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('PostV1ShopsShopIdProducts200ResponseAllOfImagesInner', function() {
    it('should create an instance of PostV1ShopsShopIdProducts200ResponseAllOfImagesInner', function() {
      // uncomment below and update the code to test PostV1ShopsShopIdProducts200ResponseAllOfImagesInner
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be.a(SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner);
    });

    it('should have the property id (base name: "id")', function() {
      // uncomment below and update the code to test the property id
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be();
    });

    it('should have the property cloudflareImageId (base name: "cloudflare_image_id")', function() {
      // uncomment below and update the code to test the property cloudflareImageId
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be();
    });

    it('should have the property createdAt (base name: "created_at")', function() {
      // uncomment below and update the code to test the property createdAt
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be();
    });

    it('should have the property updatedAt (base name: "updated_at")', function() {
      // uncomment below and update the code to test the property updatedAt
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be();
    });

    it('should have the property deletedAt (base name: "deleted_at")', function() {
      // uncomment below and update the code to test the property deletedAt
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be();
    });

    it('should have the property shopId (base name: "shop_id")', function() {
      // uncomment below and update the code to test the property shopId
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be();
    });

    it('should have the property pivot (base name: "pivot")', function() {
      // uncomment below and update the code to test the property pivot
      //var instance = new SellAuth.PostV1ShopsShopIdProducts200ResponseAllOfImagesInner();
      //expect(instance).to.be();
    });

  });

}));
